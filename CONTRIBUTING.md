...
## 디버깅 로그 이슈 (계정 공용 기본값, 2026-09-20 추가)

원인을 규명하고 수정까지 끝난 디버깅 경험은 `Debugging Log` 이슈
템플릿(`.github/ISSUE_TEMPLATE/debugging-log.yml`)으로 기록한다 —
위 "과정 기록 이슈"와 별개다: 과정 기록은 "이 작업을 왜 이렇게
진행했는가"(작업 단위 1개당 이슈 1개, 턴 코멘트로 누적)를 남기고,
디버깅 로그는 "이 증상은 이 원인이었다"는 **재사용 가능한 단일
사실**을 남긴다 — 나중에 다른 저장소·다른 작업에서 비슷한 증상을
만났을 때 검색해서 바로 참고하기 위함이다.

- **기록 대상**: 원인규명+수정이 끝난 사례만. 아직 조사 중인 버그는
  일반 `[Bug]` 이슈를 쓴다. 사소한 오탈자·자명한 실수는 기록하지
  않는다 — 다음에 시간을 아낄 만한 사례만 남긴다(신호 대비 잡음
  관리).
- **어디에 남기는가**: 문제가 실제로 발생한 저장소에 남긴다(과정
  기록 이슈와 동일 원칙 — 저장소 컨텍스트를 유지한다). 별도 중앙
  저장소로 모으지 않는다.
- **라벨**: `debugging-log` — 위 "이슈 등록 규칙"의 `type:*`(10종
  고정) taxonomy와는 **독립적으로 관리**한다. 같은 이슈에
  `type:bug` 등 기존 라벨과 함께 붙여도 된다. `type:*`를 11번째로
  늘리지 않는 이유: `type:*`는 제목 접두어와 1:1로 묶인 고정
  분류라, 검색 편의를 위한 태그(`debugging-log`)를 여기 섞으면
  둘의 개정 주기가 얽힌다.
- **검색**: `gh search issues --owner multicg --label debugging-log
  "키워드"`로 계정 전체 저장소를 가로질러 찾는다.
- **AI 에이전트 트리거**: 실질적인 버그를 원인규명+수정까지 끝낸
  직후, 사소한 건이 아니라고 판단되면 이 템플릿으로 기록할지
  사용자에게 제안한다(자동으로 만들지 않는다 — 매번 확인받는다).
- 최초 사례: [novel#264](https://github.com/multicg/novel/issues/264)
  (`verify_situation_consistency_v1` LLM 검증 체크 신설 계기).
