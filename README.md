...
| 디버깅 로그 이슈 템플릿 + `debugging-log` 라벨(2026-09-20 추가) | `.github/ISSUE_TEMPLATE/debugging-log.yml`, `labels.yml` | 위 이슈 템플릿·라벨 taxonomy와 같은 방식(수동 복사 / `apply-labels.sh`) — `type:*` taxonomy와는 독립 관리. 상세는 [CONTRIBUTING.md](./CONTRIBUTING.md) "디버깅 로그 이슈" 참고 |
